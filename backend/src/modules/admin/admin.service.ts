...
export class AdminService {
  ...
  async getDashboardStats() {
    ...
    const totalPdf = await this.chapterRepo.count({
      where: { resourceType: ILike('PDF') },
    });
    const totalVideos = await this.chapterRepo.count({
      where: { resourceType: ILike('Video') },
    });
    const totalAudio = await this.chapterRepo.count({
      where: { resourceType: ILike('Audio') },
    });
      ...
      students.map(async (s) => {
        ...
        const progressByBook = await Promise.all(
          Object.keys(booksMap).map(async (bookId) => {
            ...
            const book = userActivities.find(
              (a) => a.book?.id === +bookId,
            )?.book;
