...
export default function StudentAIAssistant() {
  ...
  const sendMessage = async () => {
    ...
    setMessages((prev) => [
      ...prev,
      { sender: "user", text: userText, suggestions: [] },
    ]);
    setLoading(true);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "bot", text: text.answer, suggestions: text.suggestions },
      ]);
      speakText(text.answer);
    } catch (err) {
      console.error(err);
      setMessages((prev) => [
        ...prev,
        { sender: "bot", text: "⚠️ AI error", suggestions: [] },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex flex-col flex-1 ml-0 lg:ml-64 bg-cardBg">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((msg, idx) => (
            <div key={idx} className="space-y-2">
              {/* Message bubble */}
              <div
                className={`max-w-[80%] px-3 py-2 rounded-lg text-sm ${msg.sender === "user" ? "bg-primaryBlue text-white ml-auto" : "bg-gray700 text-gray300"}`}
              >
                {msg.text}
              </div>

              {/* Suggested video cards (no API key) */}
              {msg.suggestions?.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                  {msg.suggestions.map((s, i) => {
                    const imgQuery = encodeURIComponent(s.query);
                    return (
                      <a
                        key={i}
                        href={`https://www.youtube.com/results?search_query=${imgQuery}`}
                        target="_blank"
                        rel="noreferrer"
                        className="bg-gray700 rounded-xl overflow-hidden shadow hover:shadow-lg transition"
                      >
        ...
        {/* Input area */}
        <div className="p-3 flex items-center gap-2 bg-gray800">
          <input
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && sendMessage()}
          ...
          <button
            onClick={isListening ? stopListening : startListening}
            className={`px-3 py-2 rounded ${isListening ? "bg-red-600" : "bg-gray700"}`}
          >
            {isListening ? <FaMicrophoneSlash /> : <FaMicrophone />}
          </button>
          <button
