...
export default function UploadChapter() {
  ...
  const toggleChapter = (id) => setOpenChapterId(openChapterId === id ? null : id);
  ...
  const handleAddPart = async (parentId) => {
    if (!partResourceType || (!file && !videoUrl && !simulationUrl)) return toast.warning("Add part details");

    try {
      const fd = new FormData();
      fd.append("partNumber", (chapters.find(c => c.id === parentId)?.parts?.length || 0) + 1);
      fd.append("resourceType", partResourceType);
      if (thumbnail) fd.append("thumbnail", thumbnail);
      if (partResourceType === "video") fd.append("videoUrl", videoUrl);
      else if (partResourceType === "simulation") fd.append("simulationUrl", simulationUrl);
      else fd.append("file", file);

      const res = await fetch(`${API_URL}/books/${bookId}/chapters/${parentId}/parts`, {
        method: "POST",
        body: fd,
        credentials: "include",
      });
      ...
      setChapters((prev) =>
        prev.map((ch) =>
          ch.id === parentId ? { ...ch, parts: [...(ch.parts || []), newPart] } : ch
        )
      );
      toast.success("Part added");
      setFile(null); setThumbnail(null); setVideoUrl(""); setSimulationUrl(""); setPartResourceType("");
      setAddPartChapterId(null);
    } catch {
      toast.error("Part upload failed");
    }
            ...
            <li key={c.id} className="py-2">
                ...
                <div className="ml-6 mt-2 space-y-1">
                  {c.parts && c.parts.map((p) => (
                    <div key={p.id} className="flex justify-between items-center text-sm">
                      <span>{c.resourceType==="video" ? `Lecture ${p.partNumber}` : `Part ${p.partNumber}`}</span>
                      {p.videoUrl && <a href={p.videoUrl} target="_blank" className="text-blue-400 underline">Watch</a>}
                      {p.simulationUrl && <a href={p.simulationUrl} target="_blank" className="text-green-400 underline">Simulation</a>}
                    </div>
                  ))}
                </div>
              )}
              ...
              {addPartChapterId===c.id && (
                <div className="ml-6 mt-2 p-3 bg-[#2f3042] rounded-lg space-y-2">
                  <select value={partResourceType} onChange={(e)=>setPartResourceType(e.target.value)} className="w-full p-2 rounded-lg bg-[#2a2b39] border border-gray-500 text-white">
                    <option value="">Select Resource Type</option>
                    {resourceTypes.map((r)=>(
                      <option key={r.id} value={r.text.toLowerCase()}>{r.text}</option>
                    ))}
                  </select>
                  {partResourceType==="video" && <input type="text" placeholder="Video URL" value={videoUrl} onChange={(e)=>setVideoUrl(e.target.value)} className="w-full p-2 rounded-lg bg-[#2a2b39] border border-gray-500 text-white"/>}
                  {partResourceType==="simulation" && <input type="text" placeholder="Simulation URL" value={simulationUrl} onChange={(e)=>setSimulationUrl(e.target.value)} className="w-full p-2 rounded-lg bg-[#2a2b39] border border-gray-500 text-white"/>}
                  {(partResourceType==="pdf" || partResourceType==="audio") && <input type="file" onChange={(e)=>setFile(e.target.files[0])} className="w-full p-2 rounded-lg bg-[#2a2b39] border border-gray-500 text-white"/>}
                  <input type="file" accept="image/*" onChange={(e)=>setThumbnail(e.target.files[0])} className="w-full p-2 rounded-lg bg-[#2a2b39] border border-gray-500 text-white"/>
                  <button onClick={()=>handleAddPart(c.id)} className="bg-blue-500 px-3 py-1 rounded-lg w-full">Upload Part</button>
                </div>
              )}
            </li>
          ))}
